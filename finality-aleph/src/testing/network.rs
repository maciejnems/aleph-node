use crate::{
    ...
    network::{
        ...
        Recipient, RequestBlocks,
    },
    AuthorityId, AuthorityKeystore, SessionId,
};
use aleph_bft::{Index, KeyBox as _, MultiKeychain, NodeIndex};
...
enum TestNetworkCommand<B: BlockT> {
    ...
    SetSyncForkRequest(B::Hash, NumberFor<B>),
}
...
impl<B: BlockT> RequestBlocks<B> for TestNetwork<B> {
    ...
    fn request_stale_block(&self, hash: B::Hash, number: NumberFor<B>) {
        self.command_tx
            .unbounded_send(TestNetworkCommand::SetSyncForkRequest(hash, number))
            .unwrap();
...
async fn prepare_one_session_test_data() -> TestData {
    let authority_names = ["//Alice", "//Bob", "//Charlie"];
    let authorities = generate_authorities(&authority_names).await;
